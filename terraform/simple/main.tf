...
resource "google_compute_instance" "w3" {

  name            = "worker-3-simple"
  hostname        = "w3-simple.cka"
  machine_type    = "e2-small"
  can_ip_forward  = true
  tags            = ["http-server"]
...
output "all_instance_ips" {
  value = {
    ...
    worker_3     = google_compute_instance.w3.network_interface[0].access_config[0].nat_ip
  }
